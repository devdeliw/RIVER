use river::interpolation::linear::{interpolate, LinearCfg};
...
fn basic_line() -> RiverResult {
    ...
    assert_eq!(rep.evaluated.len(), 2);
    assert!(approx_eq(rep.evaluated[0], 2.0));
    assert!(approx_eq(rep.evaluated[1], 4.0));
    Ok(())
}
...
fn unequal_length_error() {
    let x  = [0.0, 1.0, 2.0];
    let y  = [0.0, 1.0];
    let cfg = LinearCfg::new().set_x(&x).unwrap();
...
fn many_points() -> RiverResult {
    ...
    let x_eval_for_cfg = [0.0, 0.5, 1.0, 2.0, 5.0, 10.0];

    let y_ref  = [0.0, 2.0, 3.0, 3.0, 8.0];
    let n_eval = x_eval_for_cfg.len();
    ...
    let rep = interpolate(cfg)?;
    assert_eq!(rep.n_evaluated, n_eval);
    assert_eq!(rep.evaluated.len(), n_eval);
    assert!(approx_eq(rep.evaluated[0], y_ref[0]));
    assert!(approx_eq(rep.evaluated[2], y_ref[1]));
    assert!(approx_eq(rep.evaluated[5], y_ref[4]));
    Ok(())
}
