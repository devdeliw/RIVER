...
use numena::root_finding::common::{Algorithm, RootFindingError, Termination, ToleranceReason};
use numena::root_finding::regula_falsi::{regula_falsi, RegulaFalsiCfg, RegulaFalsiError};

type TestResult = Result<(), RegulaFalsiError>;

fn variants() -> Vec<(Algorithm, &'static str, &'static str)> {
    vec![
        (Algorithm::RegulaFalsiPure,           "pure",     Algorithm::RegulaFalsiPure.algorithm_name()),
        (Algorithm::RegulaFalsiIllinois,       "illinois", Algorithm::RegulaFalsiIllinois.algorithm_name()),
        (Algorithm::RegulaFalsiPegasus,        "pegasus",  Algorithm::RegulaFalsiPegasus.algorithm_name()),
        (Algorithm::RegulaFalsiAndersonBjorck, "ab",       Algorithm::RegulaFalsiAndersonBjorck.algorithm_name()),
    ]
}

fn base_cfg(a: Algorithm) -> RegulaFalsiCfg {
    RegulaFalsiCfg::new().with_algorithm(a)
}
...
fn finds_sqrt_2_all_variants() -> TestResult {
    ...
    for (alg, tag, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(tol).with_abs_x(tol).with_rel_x(1e-5);
        ...
        assert_bracket_invariant(&f, res.left().unwrap(), res.right().unwrap());
    }
    Ok(())
}
...
fn finds_linear_root_all_variants() -> TestResult {
    ...
    for (alg, tag, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(tol).with_abs_x(tol).with_rel_x(0.0).with_max_iter(10);
...
fn no_sign_change_all_variants() {
    let f = |x: f64| x * x + 1.0;
    for (alg, tag, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1e-10);
...
fn endpoint_a_is_root_iterations_0_all_variants() -> TestResult {
    let f = |x: f64| x;
    for (alg, _, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1e-12);
...
fn uses_max_iter_1_hits_limit_all_variants() -> TestResult {
    let f = |x: f64| x * x - 2.0;
    for (alg, _, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1e-30).with_abs_x(0.0).with_rel_x(1e-16).with_max_iter(1);
...
fn narrow_interval_stops_on_width_all_variants() -> TestResult {
    let f = |x: f64| x + 1e-16;
    for (alg, _, _) in variants() {
        let cfg = base_cfg(alg).with_abs_x(1e-12).with_rel_x(0.0).with_abs_fx(1e-20);
...
fn high_function_tol_stops_quickly_all_variants() -> TestResult {
    let f = |x: f64| x;
    for (alg, _, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1.0).with_max_iter(5);
...
fn high_rel_tol_ignores_abs_x_all_variants() -> TestResult {
    ...
    for (alg, _, _) in variants() {
        let cfg = base_cfg(alg).with_abs_x(1e-12).with_rel_x(0.5).with_max_iter(100);
...
fn degenerate_secant_fallback_is_safe_all_variants() -> TestResult {
    ...
    for (alg, _, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1e-30).with_abs_x(1e-30).with_rel_x(0.0).with_max_iter(10);
...
fn algorithm_field_contains_variant_tag() -> TestResult {
    let f = |x: f64| x * x - 2.0;
    for (alg, _, expected) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1e-12).with_abs_x(1e-12);
        let res = regula_falsi(&f, 0.0, 2.0, cfg)?;
...
fn convex_pathology_orders_variants_by_speed() -> TestResult {
    ...
    for (alg, tag, _) in variants() {
        let cfg = base_cfg(alg).with_abs_fx(1e-12).with_abs_x(1e-12).with_rel_x(0.0).with_max_iter(500);
        let res = regula_falsi(&f, 0.0, 1.0, cfg)?;
...
fn final_bracket_still_straddles_root_all_variants() -> TestResult {
    ...
    for (alg, _, _) in variants() {
        ...
        assert!(res.left().unwrap() >= a0 && res.right().unwrap() <= b0);
        assert_bracket_invariant(&f, res.left().unwrap(), res.right().unwrap());
        assert!(res.root() >= res.left().unwrap() && res.root() <= res.right().unwrap());
    }
    Ok(())
}
...
fn invalid_tolerance_is_reported_all_variants() {
    let f = |x: f64| x;
    for (alg, tag, _) in variants() {
        let cfg = base_cfg(alg).with_abs_x(f64::NAN);
        let err = regula_falsi(&f, -1.0, 1.0, cfg).unwrap_err();
...
fn pure_variant_reaches_iteration_limit_on_quadratic() -> TestResult {
    let f = |x: f64| x * x - 2.0;
    let cfg = base_cfg(Algorithm::RegulaFalsiPure)
        .with_abs_fx(1e-30)
        .with_abs_x(0.0)
        .with_rel_x(1e-16)
        .with_max_iter(25);
